...
送信エラーが記録されないのにメールが届かない場合は、管理画面の `通知診断ログ` を一時的にONにしてください。ONの間は、Mailerで例外が出なかった送信試行も「最近のエラー」に記録します。調査後はログが増えすぎないようOFFに戻してください。

管理画面の `通知テスト送信` では、いいね履歴や件数を増やさずに、選択中フォームの管理者宛メール設定だけを使ってテストメールを送信できます。フォームごとの配送差分を確認したい場合に利用してください。
...
`通知診断ログ` がONの場合は、送信試行成功も記録します。このログは「DFいいねからMailerへ渡したところまでは例外が出ていない」ことを確認するためのもので、メールボックスへの到達を保証するものではありません。
